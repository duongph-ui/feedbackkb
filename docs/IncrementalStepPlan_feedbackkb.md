...
**Components touched**:
- `.claude/skills/capture-fix/SKILL.md` (new), `agents/hooks/capture-lesson.sh` (new)
...
**Components touched**:
- `agents/fbk-conductor.md` (new), `packages/server/feedbackkb_server/service/queue.py` (claim/lease/reaper) (new)
...
**Components touched**:
- `agents/lib/ground_context.md` (shared prompt module) hoặc `packages/server/.../grounding.py` (new)
...
**Components touched**:
- `agents/fbk-fixer.md` (new), `agents/lib/sandbox.md` (ràng buộc §7.4) (new)
...
**Components touched**:
- `agents/lib/classify_target.md` / `service/classify.py` (new)
...
**Components touched**:
- `agents/fbk-analyst.md` (mở rộng pha impact), `.github/workflows/fix-ci.yml` (CI gate nhánh fix) (new)
...
**Components touched**:
- `agents/fbk-conductor.md` (wire fix loop), `service/approval.py` (new)
...
```
DAG — không cycle (codex P1 fix: [27] trước [26]; [11] cần [10]; [21a]→[21b]; [36/38/41] thêm dep CLI/widget). 44 step (42 + 13b + 21 tách 21a/21b).
...
| `agents/fbk-{conductor,triage,analyst,fixer}.md` | 28,26,31/34,32 |
| `agents/lib/{ground_context,sandbox,classify_target}.md` | 30,32,33 |
